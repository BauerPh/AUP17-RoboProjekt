...
Public Class classCommunication
    ...
    Public Sub connect(comPort As String)
        ...
        Catch ex As Exception
            OnLog(New LogEventArgs("Verbindungsfehler!", classLogger.errLvl.ERR))
            disconnect()
            Return
        End Try
    ...
    Public Sub disconnect()
        ...
        OnLog(New LogEventArgs("Verbindung getrennt!", classLogger.errLvl.INFO))
    End Sub
    ...
    Private Sub _tConWaitACK_Elapsed(sender As Object, e As ElapsedEventArgs) Handles _tConWaitACK.Elapsed
        ...
        If _repeatCounter >= _cSendRepeats Then
            ...
            disconnect()
            _conWaitACK = False
            OnLog(New LogEventArgs("Falsches Gerät angeschlossen!", classLogger.errLvl.ERR))
        Else
            sendCON()
        End If
    End Sub
    Private Sub _tMovWaitACK_Elapsed(sender As Object, e As ElapsedEventArgs) Handles _tWaitACK.Elapsed
        ...
        If _repeatCounter >= _cSendRepeats Then
            ...
            _movWaitACK = False
            _refWaitACK = False
            OnLog(New LogEventArgs("keine Antwort!", classLogger.errLvl.ERR))
        Else
            If _movWaitACK Then
                sendMOV()
    ...
    Private Sub rcvMsg(msg As String)
        OnLog(New LogEventArgs(msg, classLogger.errLvl.COMIN))
    ...
    Private Sub rcvACK()
        If _conWaitACK Then
            ...
            OnLog(New LogEventArgs("Verbunden!", classLogger.errLvl.INFO))
    ...
    Private Sub rcvERR()
        ...
        _tWaitACK.Stop()
        If errnum = 1 Then
            OnLog(New LogEventArgs("Error!", classLogger.errLvl.ERR))
        ElseIf errnum = 2 Then
            OnLog(New LogEventArgs("Parameter Error!", classLogger.errLvl.ERR))
        ElseIf errnum = 3 Then
            OnLog(New LogEventArgs("Referenz fehlt!", classLogger.errLvl.ERR))
        ElseIf errnum = 4 Then
            OnLog(New LogEventArgs("Referenz fehlgeschlagen!", classLogger.errLvl.ERR))
        End If
    ...
    Private Sub sendMsg(msg As String)
        ...
        OnLog(New LogEventArgs(msg, classLogger.errLvl.COMOUT))
    End Sub
...
'Event Parameter
Public Class LogEventArgs : Inherits EventArgs
    Private _LogMsg As String
    Private _LogLvl As String

    Public Sub New(LogMsg As String, LogLvl As String)
        _LogMsg = LogMsg
        _LogLvl = LogLvl
    End Sub
    Public ReadOnly Property LogMsg As String
        ...
        End Get
    End Property
    Public ReadOnly Property LogLvl As String
        Get
            Return _LogLvl
        End Get
    End Property
